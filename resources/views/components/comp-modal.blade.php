<div>
    ...
    <div class="{{ $this->showModal }} relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        @php
        $icon = $this->mode ? "fa-solid fa-edit": "fa-solid fa-plus";
        @endphp
        <!--
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-700  text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                    <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center sm:mx-0 sm:h-10 sm:w-10 {{ $icon }}">
                            </div>
                            {{ $slot ?? null }}
                        </div>
                    </div>
                    <div class=" px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <button type="submit" class="inline-flex w-full justify-center rounded-md border border-transparent bg-green-600 px-4 py-2 text-base font-medium shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm">{{$this->mode ? "Actualizar": "Guardar"}}
                        </button>
