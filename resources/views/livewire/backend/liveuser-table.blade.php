    ...
    <x-forms.table caption="Usuarios">
            ...
            <tr>
                ...
                <th class="flex justify-between px-6 py-3 col-span-2 gap-2">
                    ...
                    <div class="justify-end">
                        <button wire:click="fncNewEdit()" class="btn btn-blue text-xs"><i class="fa-solid fa-user-plus"></i>
                            {{ __($display['new']) }}
                        </button>
                        {{-- @endhasanyrole --}}
                    </div>
        ...
        <tr class="bg-gray-{{ $cl }} dark:bg-gray-{{ $cl == '50' ? $cl + 450 : $cl + 400 }} border-b hover:bg-gray-300 dark:border-gray-700 dark:hover:bg-gray-600">
            ...
            <td class="whitespace-nowrap px-2 py-4 text-sm text-gray-500">
                ...
                <img class="h-10 w-10 rounded-full" src="{{ $reg->profile_photo_path }}" alt="avatar">
                @else
                <img class="h-10 w-10 rounded-full" src="{{ asset($reg->profile_photo_path) }}" alt="avatar">
                @endif
                @break
            ...
            <td class="flex px-6 py-4 col-span-2 justify-around gap-2">
                ...
                <div>
                    {{-- @hasanyrole('admin') --}} <button wire:click="fncDeleteConfirm({{ $reg->id }})" wire:loading.attr="disabled" class="btn btn-red justify-between text-xs"><i class="fa-solid fa-user-minus"></i>
                        {{ __($display['delete']) }}
                    </button>
                    {{-- @endhasanyrole --}}
                </div>
